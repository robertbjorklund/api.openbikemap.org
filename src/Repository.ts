...
export class Repository {
  ...
  getBySourceId = async (
    ...
    if (result.rows.length === 0) {
      throw new Error(
        `Feature with source ${sourceType}/${sourceId} not found`,
      );
    }
    ...
    return this.pool.query(
      `SELECT id, type, geometry, properties FROM (
         SELECT
           ...
           END AS name_score,
           CASE
             WHEN type = 'route' THEN 2
             WHEN type = 'trail' THEN 1
             ELSE 0
           END AS type_score
         FROM features
  ...
  removeExceptImport = async (importID: string): Promise<void> => {
    await this.pool.query("DELETE FROM features WHERE import_id != $1", [
      importID,
    ]);
  };
  ...
  upsert = async (feature: Feature, importID: string): Promise<void> => {
    ...
    await this.pool.query("DELETE FROM feature_sources WHERE feature_id = $1", [
      id,
    ]);
    ...
    if (sources.length > 0) {
      ...
      const params: unknown[] = [
        id,
        ...sources.flatMap((s) => [s.type, String(s.id)]),
      ];
      await this.pool.query(
...
function getSearchableText(feature: Feature): string {
  const parts: (string | null | undefined)[] = [
    feature.properties.name,
    feature.properties.ref,
  ];

  if ("network" in feature.properties) {
    parts.push(feature.properties.network);
  }

  return [...new Set(parts.filter((v): v is string => !!v))].join(" ");
}
